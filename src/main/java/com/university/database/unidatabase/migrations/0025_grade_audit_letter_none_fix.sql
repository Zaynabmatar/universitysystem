-- Fixes trg_Grade_Audit (0023_grade_audit_detail.sql, 0024_grade_audit_grade_points.sql) writing
-- "letter=(n" instead of "letter=(none)" whenever letter_grade goes to/from NULL.
--
-- dbo.grades.letter_grade is NVARCHAR(2) ('A-', 'B+', 'F', ...). ISNULL(d.letter_grade, N'(none)')
-- takes its result type from the FIRST typed argument, so the whole expression is NVARCHAR(2) and
-- the six-character literal '(none)' gets silently cut down to '(n' -- confirmed with
-- SELECT ISNULL(CAST(NULL AS NVARCHAR(2)), N'(none)') = '(n' on this server. Every other tracked
-- column already avoids this: the numeric marks are CONVERT(NVARCHAR(20), ...)'d to a wide type
-- before the ISNULL runs, so only letter_grade -- the one column passed to ISNULL directly -- was
-- ever affected. COALESCE does not have this quirk (it widens to fit every argument), so this
-- migration only swaps ISNULL for COALESCE on the two letter_grade branches; every other line of
-- the trigger is byte-for-byte what 0024 left it as.
--
            ...
            SELECT
                old_value = CONCAT(
                    ...
                    CASE WHEN ISNULL(i.letter_grade, N'~') <> ISNULL(d.letter_grade, N'~')
                         THEN CONCAT(N'letter=', COALESCE(d.letter_grade, N'(none)'), N'; ') ELSE N'' END,
                ...
                new_value = CONCAT(
                    ...
                    CASE WHEN ISNULL(i.letter_grade, N'~') <> ISNULL(d.letter_grade, N'~')
                         THEN CONCAT(N'letter=', COALESCE(i.letter_grade, N'(none)'), N'; ') ELSE N'' END,
                    CASE WHEN ISNULL(i.grade_points, -1) <> ISNULL(d.grade_points, -1)
