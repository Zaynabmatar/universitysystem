-- Fixes the Admin Activity Log showing meaningful detail for some grade edits and nothing useful
-- for others.
--
-- trg_Grade_Audit (phase11_grades_scale.sql) has always logged only total_mark/letter_grade/
-- grade_points/is_submitted -- never the raw coursework_mark/midterm_mark/lab_mark/final_mark
-- columns an instructor actually types into. Those four summary columns stay NULL until every
-- required component has been entered (GradeCalculator.totalMark), so any instructor who saves
-- marks gradually -- Midterm today, Final next week, the normal "Save Draft" workflow -- produces
-- audit rows whose old_value/new_value are both "total=; letter=; points=; submitted=0": identical
-- before and after, so the log shows an entry with no visible detail. An instructor who happens to
-- enter every mark in one sitting (so the total computes immediately) gets a readable row purely by
-- accident of timing -- same action, inconsistent detail, for no reason tied to who is instructing.
--
-- This migration replaces the trigger with one that tracks the actual mark/publish columns (the
-- fields that actually change on a Save Draft or Publish), following the same CROSS APPLY
-- changed-columns pattern every other trg_*_Audit trigger already uses (0014_audit_log_triggers.sql,
-- 0021_section_audit_trigger.sql) -- including only inserting a row when something in the tracked
-- set actually changed, which the original trigger never checked either (it logged a row on every
-- UPDATE statement, changed or not).
